...
const DID_YOU_KNOW = [
  { slug: 'civil', emoji: '🏗️', title: 'Civil Engineering', hashtag: 'CivilEngineering', search: 'bridge construction engineering',
    body: 'Civil engineers design and maintain the built environment — roads, bridges, dams, tunnels and the water systems entire cities depend on. In Uzbekistan the discipline carries a special seismic duty: after the devastating 1966 earthquake leveled much of old Tashkent, the capital was rebuilt almost from scratch with earthquake resistance built into every structure. Modern buildings here use base isolation and ductile framing — designs that let a structure flex and absorb shaking instead of cracking under it — the same techniques now standard across seismically active Central Asia. Every bridge, dam and high-rise you pass in Tashkent has survived a seismic design check most cities never have to think about.' },
  { slug: 'electrical', emoji: '⚡', title: 'Electrical Engineering', hashtag: 'ElectricalEngineering', search: 'electrical grid power lines',
    body: 'Electrical engineers plan and run everything from national power grids down to the switch on your wall. The core idea behind almost all of it is deceptively simple: a closed loop. Current only flows when there’s an unbroken path from source, through a switch and a load, and back again — break that loop anywhere and the whole system goes dark. Substations, transmission lines and the wiring in your house are all built around managing that one rule at different scales. Uzbekistan’s grid is now going through a major modernization push, integrating new solar capacity into a transmission network originally built for a very different energy mix — one of the more active areas for electrical engineers here today.' },
  { slug: 'mechanical', emoji: '⚙️', title: 'Mechanical Engineering', hashtag: 'MechanicalEngineering', search: 'gears machine engineering',
    body: 'Mechanical engineers work with forces, motion and energy — engines, machines, manufacturing lines, anything that moves. It’s the broadest of the classical engineering disciplines, and machine design underpins the hardware inside nearly every other field, from the robots built by mechatronics engineers to the pumps that keep a chemical plant running. One of the field’s oldest tricks: gear trains. Every meshing pair of gears reverses direction, so an odd-numbered gear in a chain always spins the same way as the first — a simple counting rule that lets engineers trade speed for torque, scaling from a wristwatch’s tiny gears to the hoists that lift entire mineshafts.' },
  { slug: 'industrial', emoji: '🏭', title: 'Industrial Engineering', hashtag: 'IndustrialEngineering', search: 'factory assembly line',
    body: 'Industrial engineers don’t design the machines themselves — they design how people, machines and materials work together: factory layout, logistics, quality systems. Their core tool isn’t a single piece of hardware but statistics and process design. The single biggest idea in the field is the bottleneck: a production line can never run faster than its slowest station, no matter how fast the others go — the rest just build up queues waiting on it. Finding and widening that one constraint, rather than speeding up whatever’s already fast, is the entire economy of industrial engineering.' },
  { slug: 'chemical', emoji: '🧪', title: 'Chemical Engineering', hashtag: 'ChemicalEngineering', search: 'chemical plant industrial',
    body: 'Chemical engineers scale reactions from a lab beaker up to full industrial plants — fuels, fertilizers, polymers, pharmaceuticals all pass through their hands before reaching anyone else. The discipline runs on two things above all: thermodynamics and process safety. Even the simplest reaction has to balance exactly — burn methane (CH₄) and you need precisely two O₂ molecules to produce one CO₂ and two H₂O, atom for atom. Multiply that bookkeeping by a plant producing thousands of tonnes a day, running at high pressure and temperature, and you understand why chemical engineers spend as much time on safety margins as they do on yield.' },
  { slug: 'aerospace', emoji: '✈️', title: 'Aerospace Engineering', hashtag: 'AerospaceEngineering', search: 'aircraft airplane engineering',
    body: 'Aerospace engineers design aircraft and spacecraft, where every gram and every joint is calculated down to the decimal. The discipline’s signature shape is the airfoil — a wing cross-section that turns moving air into lift. Lift grows as the wing’s angle of attack increases, right up until a critical point, usually around 15°, where the airflow can no longer follow the wing’s upper surface. It separates, lift collapses, and the wing stalls. That narrow margin between useful lift and stall drives almost every decision in aircraft design — wing shape, flight envelopes, and the approach speed of every landing you’ve ever been on.' },
  { slug: 'environmental', emoji: '🌊', title: 'Environmental Engineering', hashtag: 'EnvironmentalEngineering', search: 'water treatment plant',
    body: 'Environmental engineers protect water, air and soil — treatment plants, emissions control, contamination cleanup. In Central Asia the discipline is inseparable from one resource above all: water. Irrigation efficiency and the ongoing effort to stabilize the shrinking Aral Sea basin are central to the region’s environmental engineering work. Even something as basic as a sand filter reflects careful design: water flows downward through layers ordered fine-to-coarse, so the finest sand sits on top and traps particles right at the surface where the filter can be cleaned, while coarser layers below carry the water away without clogging.' },
  { slug: 'software', emoji: '💻', title: 'Software Engineering', hashtag: 'SoftwareEngineering', search: 'computer programming code',
    body: 'Software engineering applies the same discipline as any other engineering field — specification, testing, verification — to code instead of concrete or steel. As more infrastructure becomes computer-controlled, from traffic systems to power grids, a software defect stops being just an inconvenience and starts being a civil-safety issue. Most production failures trace back to something tiny: an off-by-one error, a boundary condition nobody tested, one bad read that poisons an entire result. Catching those systematically — through code review, automated testing, and designing around invariants that must never break — rather than relying on luck, is what separates software engineering from just writing code.' },
  { slug: 'robotics', emoji: '🤖', title: 'Robotics', hashtag: 'Robotics', search: 'industrial robot arm',
    body: 'Robotics and mechatronics combine mechanics, electronics and control software into machines that can sense their surroundings and act on their own. Every robot, no matter how advanced, runs the same basic control loop thousands of times per second: sense the world, plan a response, then act. Acting on stale information — skipping straight to ‘act’ without first sensing — is how robots crash or collide. That sense-plan-act loop is the shared skeleton behind a factory arm welding car frames, a warehouse rover picking orders, and a self-driving car reading the road ahead of it.' },
  { slug: 'structural', emoji: '🏛️', title: 'Structural Engineering', hashtag: 'StructuralEngineering', search: 'steel truss bridge structure',
    body: 'Structural and architectural engineers make buildings stand — sizing every beam, column and foundation against gravity, wind and, in this region, earthquakes. One of the field’s most elegant ideas is also one of its oldest: the triangle. A triangle can’t change shape without changing the length of one of its sides, so its joints carry no bending force at all — unlike a square, which simply folds into a parallelogram under the same push. That single geometric fact is why trusses, cranes and transmission towers are built almost entirely out of triangles: it’s the strongest shape you can make from the least material.' }
];
